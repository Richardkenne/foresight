...
function DomainChart({ profile }: { profile: SacredProfile }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
      {domainScores.map(d => (
        <div key={d.domain} style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
    ...
    return (
      <div style={{ display: 'flex', flexDirection: 'column', height: '100%', overflow: 'hidden' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 16px', borderBottom: '1px solid var(--border)' }}>
          <button onClick={onClose} style={{ display: 'flex', alignItems: 'center', gap: '6px', background: 'none', border: 'none', color: 'var(--foreground)', fontSize: '13px', fontWeight: 600, cursor: 'pointer', padding: 0 }}>
          ...
          <span style={{ fontSize: '9px', fontWeight: 700, padding: '2px 8px', borderRadius: '10px', letterSpacing: '0.06em', fontFamily: 'var(--font-geist-mono, monospace)', background: 'rgba(16,185,129,0.1)', color: 'var(--success)' }}>
            COMPLETE
          </span>
        </div>
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '16px' }}>
          ...
          <div style={{ display: 'flex', gap: '12px', marginBottom: '20px' }}>
            <div style={{ flex: 1, padding: '10px 12px', borderRadius: '8px', background: 'rgba(16,185,129,0.06)', border: '1px solid rgba(16,185,129,0.15)' }}>
              <div style={{ fontSize: '9px', fontWeight: 700, color: 'var(--success)', letterSpacing: '0.06em', marginBottom: '4px', fontFamily: 'var(--font-geist-mono, monospace)' }}>STRONGEST</div>
              {top3.map(([id, score]) => (
            ...
            <div style={{ flex: 1, padding: '10px 12px', borderRadius: '8px', background: 'rgba(239,68,68,0.06)', border: '1px solid rgba(239,68,68,0.15)' }}>
              <div style={{ fontSize: '9px', fontWeight: 700, color: 'var(--danger)', letterSpacing: '0.06em', marginBottom: '4px', fontFamily: 'var(--font-geist-mono, monospace)' }}>GROWTH AREAS</div>
          ...
          {/* Domain breakdown */}
          <div style={{ fontSize: '9px', fontWeight: 700, color: 'var(--muted)', letterSpacing: '0.06em', textTransform: 'uppercase' as const, marginBottom: '10px', fontFamily: 'var(--font-geist-mono, monospace)' }}>
            DOMAIN BREAKDOWN
          </div>
        ...
        <div style={{ padding: '10px 16px', borderTop: '1px solid var(--border)', display: 'flex', gap: '8px' }}>
          <button onClick={handleRetake} style={{
            flex: 1, padding: '8px', border: '1px solid var(--border)', borderRadius: '6px',
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', overflow: 'hidden' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 16px', borderBottom: '1px solid var(--border)' }}>
        ...
        <span style={{ fontSize: '9px', fontWeight: 700, padding: '2px 8px', borderRadius: '10px', letterSpacing: '0.06em', fontFamily: 'var(--font-geist-mono, monospace)', background: 'var(--surface-hover)', color: 'var(--muted)' }}>
          ...
          {pageQuestions.map((q, idx) => {
            ...
            return (
              <div key={q.id} style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                ...
                {/* Options */}
                <div style={{ display: 'flex', flexDirection: 'column', gap: '3px', paddingLeft: '28px' }}>
                  {q.options.map(opt => {
      ...
      {/* Footer / Navigation */}
      <div style={{ padding: '10px 16px', borderTop: '1px solid var(--border)', display: 'flex', gap: '8px', alignItems: 'center' }}>
        {currentPage > 0 && (
          <button
            onClick={() => setCurrentPage(p => p - 1)}
            style={{
              padding: '8px 16px', border: '1px solid var(--border)', borderRadius: '6px',
              background: 'none', color: 'var(--muted-foreground)', fontSize: '11px', fontWeight: 500, cursor: 'pointer',
            }}
          >
        ...
        {currentPage < totalPages - 1 ? (
          ...
          >
            Next
            <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" style={{ verticalAlign: 'middle', marginLeft: '4px' }}>
