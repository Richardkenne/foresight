      ...
      {domainScores.map(d => (
        <div key={d.domain} style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-2)' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <span style={{ fontSize: 'var(--text-sm)', fontWeight: 600, color: 'var(--foreground)' }}>{d.label}</span>
            <span style={{
              fontSize: 'var(--text-xs)', fontWeight: 700, fontFamily: 'var(--font-geist-mono, monospace)',
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '3px', paddingLeft: 'var(--space-1)' }}>
            {d.roots.filter(r => r.score > 0).map(r => (
              <span key={r.id} style={{
                fontSize: 'var(--text-xs)', padding: '1px var(--space-2)', borderRadius: '4px',
                background: r.score >= 7 ? 'var(--success-muted)' : r.score >= 4 ? 'var(--warning-muted)' : 'var(--danger-muted)',
    ...
    return (
      <div style={{ display: 'flex', flexDirection: 'column', height: '100%', overflow: 'hidden' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: 'var(--space-4) var(--space-4)', borderBottom: '1px solid var(--border)' }}>
          <button onClick={onClose} style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-2)', background: 'none', border: 'none', color: 'var(--foreground)', fontSize: 'var(--text-base)', fontWeight: 600, cursor: 'pointer', padding: 0 }}>
          ...
          <span style={{ fontSize: 'var(--text-xs)', fontWeight: 700, padding: '2px var(--space-2)', borderRadius: '10px', letterSpacing: '0.06em', fontFamily: 'var(--font-geist-mono, monospace)', background: 'var(--success-muted)', color: 'var(--success)' }}>
            COMPLETE
          </span>
        </div>
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: 'var(--space-4)' }}>
          ...
          <div style={{ display: 'flex', gap: 'var(--space-3)', marginBottom: 'var(--space-6)' }}>
            <div style={{ flex: 1, padding: 'var(--space-3) var(--space-3)', borderRadius: '8px', background: 'var(--success-muted)', border: '1px solid color-mix(in srgb, var(--success) 15%, transparent)' }}>
              <div style={{ fontSize: 'var(--text-xs)', fontWeight: 700, color: 'var(--success)', letterSpacing: '0.06em', marginBottom: 'var(--space-1)', fontFamily: 'var(--font-geist-mono, monospace)' }}>STRONGEST</div>
              {top3.map(([id, score]) => (
                <div key={id} style={{ fontSize: 'var(--text-xs)', color: 'var(--foreground)', lineHeight: 1.5 }}>
            ...
            <div style={{ flex: 1, padding: 'var(--space-3) var(--space-3)', borderRadius: '8px', background: 'var(--danger-muted)', border: '1px solid color-mix(in srgb, var(--danger) 15%, transparent)' }}>
              <div style={{ fontSize: 'var(--text-xs)', fontWeight: 700, color: 'var(--danger)', letterSpacing: '0.06em', marginBottom: 'var(--space-1)', fontFamily: 'var(--font-geist-mono, monospace)' }}>GROWTH AREAS</div>
          ...
          {/* Domain breakdown */}
          <div style={{ fontSize: 'var(--text-xs)', fontWeight: 700, color: 'var(--muted)', letterSpacing: '0.06em', textTransform: 'uppercase' as const, marginBottom: 'var(--space-3)', fontFamily: 'var(--font-geist-mono, monospace)' }}>
            DOMAIN BREAKDOWN
          </div>
        ...
        <div style={{ padding: 'var(--space-3) var(--space-4)', borderTop: '1px solid var(--border)', display: 'flex', gap: 'var(--space-2)' }}>
          <button onClick={handleRetake} style={{
            ...
            background: 'none', color: 'var(--muted-foreground)', fontSize: 'var(--text-sm)', fontWeight: 500, cursor: 'pointer',
          }}>
            Retake Assessment
          </button>
          <button onClick={onClose} style={{
            flex: 1, padding: 'var(--space-2)', border: 'none', borderRadius: '6px',
            background: 'var(--foreground)', color: 'var(--background)', fontSize: 'var(--text-sm)', fontWeight: 500, cursor: 'pointer',
          }}>
            Done
          </button>
        </div>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', overflow: 'hidden' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: 'var(--space-4) var(--space-4)', borderBottom: '1px solid var(--border)' }}>
        ...
        <span style={{ fontSize: 'var(--text-xs)', fontWeight: 700, padding: '2px var(--space-2)', borderRadius: '10px', letterSpacing: '0.06em', fontFamily: 'var(--font-geist-mono, monospace)', background: 'var(--surface-hover)', color: 'var(--muted)' }}>
            ...
            return (
              <div key={q.id} style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-2)' }}>
                ...
                <div style={{ display: 'flex', gap: 'var(--space-2)', alignItems: 'flex-start' }}>
                  <span style={{
                    fontSize: 'var(--text-xs)', fontWeight: 700, color: 'var(--muted)', fontFamily: 'var(--font-geist-mono, monospace)',
                    minWidth: '20px', paddingTop: '2px',
                  }}>
                    {String(globalIdx).padStart(2, '0')}
                  </span>
                  <div style={{ flex: 1 }}>
                    <div style={{ fontSize: 'var(--text-sm)', fontWeight: 500, color: 'var(--foreground)', lineHeight: 1.5, marginBottom: '2px' }}>
                      {q.question}
                    </div>
                    <div style={{ fontSize: 'var(--text-xs)', color: 'var(--muted)', fontFamily: 'var(--font-geist-mono, monospace)' }}>
                      {mappedRoots.join(' + ')}
                    </div>
                  </div>
                </div>
                    ...
                    return (
                      <button
                        ...
                        style={{
                          ...
                          fontSize: 'var(--text-sm)', fontWeight: isSelected ? 500 : 400,
        ...
        {currentPage < totalPages - 1 ? (
          ...
          <button
            ...
            style={{
              ...
              color: allAnswered ? 'var(--background)' : 'var(--muted)',
              fontSize: 'var(--text-sm)', fontWeight: 600, cursor: allAnswered ? 'pointer' : 'not-allowed',
