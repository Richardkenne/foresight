...
const REPORT_FILE = path.join(process.cwd(), 'data', 'backtest-report.json');
...
// GET: serve the static report file (generated by `npm run backtest`)
// Falls back to live run state if no report file exists
export async function GET() {
  // Try static report first
  ...
  // Fall back to live run state
  if (lastResults) {
    return NextResponse.json(lastResults);
  }

  return NextResponse.json(
    { error: 'No backtest report found. Run: npm run backtest' },
    { status: 404 }
  );
}
